import Excercisedata from "./assets/custom-componets/excercisedata";
import Editxcercisedata from "./assets/custom-componets/editexcercisedata";

const search =
  "data:image/png;base64,";

const Workoutlog = () => {
  return (
    <div className="w-full h-full bg-gray-400  py-4 pt-5 px-5 ">
      <div className="flex gap-4">
        <div className=" flex rounded-xl bg-white grow">
          <input
            type="text"
            className=" py-4 px-4 w-full  rounded-xl h-full "
          />
          <div className="flex justify-center items-center mr-4">
            <img src={search} alt="search icon" />
          </div>
        </div>
        <div className="w-fit h-fit p-4 bg-white rounded-xl flex">
          <img
            width="25"
            height="25"
            src="https://img.icons8.com/tiny-glyph/16/add-folder.png"
            alt="add-folder"
          />
        </div>
      </div>
      <Excercisedata />
      <Excercisedata />
      <Excercisedata />
      <Editxcercisedata />
      <Editxcercisedata />
    </div>
  );
};

export default Workoutlog;
